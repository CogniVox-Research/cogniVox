...
import { Mic, Volume2, Heart, BadgeQuestionMark } from 'lucide-react';
...
import QuestionResultsTab from './tabs/QuestionResultsTab';

const FinishedPage = ({ state }: { state: FinishedState }) => {
  const [activeTab, setActiveTab] = useState<'delivery' | 'transcript' | 'biometrics' | 'question'>('delivery');
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex gap-8">
            {[
              ...
              { id: 'question', label: 'Q & A Results', icon: BadgeQuestionMark },
            ].map((tab) => (
              <motion.button
                key={tab.id}
      ...
      <div className="max-w-7xl mx-auto p-6">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'question' && <QuestionResultsTab key="biometrics" state={state} />}
        </AnimatePresence>
      </div>
    </div>
  );
