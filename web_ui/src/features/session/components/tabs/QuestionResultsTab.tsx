...
const QuestionResultsTab = ({ state }: QnATabProps) => {
  ...
  if (!answerScore) {
    return (
      <div className="p-6 text-center text-slate-500">
        No answer score data available
      </div>
    );
  }
  ...
  const matchingQuestions = answerScore.results.filter(
    (r) => r.is_matching,
  ).length;
  ...
  const averagePercentage =
    answerScore.results.length > 0
      ? (
          answerScore.results.reduce(
            (sum, r) => sum + r.matching_percentage,
            0,
  ...
  const chartData = answerScore.results.map((result, idx) => ({
    id: idx,
    question: `Q${idx + 1}`,
    percentage: Math.round(result.matching_percentage),
  ...
  const scoreDistribution = [
    {
      ...
      count: chartData.filter((d) => d.percentage >= 80).length,
    },
    {
      range: "Good (60-79%)",
      count: chartData.filter((d) => d.percentage >= 60 && d.percentage < 80)
        .length,
    },
    {
      range: "Fair (40-59%)",
      count: chartData.filter((d) => d.percentage >= 40 && d.percentage < 60)
        .length,
    },
    {
      range: "Poor (<40%)",
      count: chartData.filter((d) => d.percentage < 40).length,
    },
  ];
  ...
  const overallScorePercent = Math.round(answerScore.overall_score);
    ...
    >
        ...
        <div className="flex items-center justify-between gap-8">
          <div className="flex-1">
            ...
            <p className="text-slate-700 mt-3 leading-relaxed">
              Your answers achieved an overall score of{" "}
              <span className="font-bold text-3xl text-blue-600">
                {overallScorePercent * 20}%
              </span>
              . This represents how well your responses matched the expected
            ...
            <div className="mt-4 flex gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-slate-700">
                  <strong>{matchingQuestions}</strong> Matching
                </span>
              </div>
              <div className="flex items-center gap-2">
                <AlertCircle className="w-5 h-5 text-red-600" />
                <span className="text-sm text-slate-700">
                  <strong>{nonMatchingQuestions}</strong> Not Matching
                </span>
              </div>
            </div>
          </div>
      ...
      >
        <div className="bg-green-50 rounded-lg p-5 border-2 border-green-200">
          ...
          <p className="text-sm text-green-700 mt-2">
            {(matchingQuestions / totalQuestions).toFixed(0)}% of total
          </p>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-5 border-2 border-red-200">
          ...
          <p className="text-4xl font-bold text-red-900">
            {nonMatchingQuestions}
          </p>
          <p className="text-sm text-red-700 mt-2">
            {(nonMatchingQuestions / totalQuestions).toFixed(0)}% of total
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {scoreDistribution.map((dist, idx) => {
            ...
            return (
              ...
              >
                <p
                  className={`text-xs font-bold ${colors.text} uppercase mb-2`}
                >
                  {dist.range}
                </p>
                ...
                <p className={`text-xs ${colors.text} mt-2`}>
                  {(dist.count / totalQuestions).toFixed(0)}% of answers
                </p>
              </motion.div>
            );
          })}
      ...
      >
        ...
        <div className="space-y-3">
          {answerScore.results.map((result, idx) => {
            const percentage = Math.round(result.matching_percentage);
              ...
              >
                <div className="flex items-start justify-between gap-4 mb-3">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p
                      className={`text-xs ${colors.accent} font-semibold mt-1`}
                    >
          ...
          {overallScorePercent >= 80 ? (
            <div className="bg-green-50 p-4 rounded-lg border border-green-300">
              ...
              <p className="text-sm text-green-800">
                Your answers demonstrate strong alignment with expected
                responses. You have clearly understood the questions and
                provided relevant, comprehensive answers.
              </p>
            </div>
          ) : overallScorePercent >= 60 ? (
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-300">
              ...
              <p className="text-sm text-blue-800">
                Your answers show good understanding of most questions. Review
                the lower-scoring answers to identify gaps in knowledge or
                comprehension.
              </p>
            </div>
          ) : overallScorePercent >= 40 ? (
            <div className="bg-amber-50 p-4 rounded-lg border border-amber-300">
              ...
              <p className="text-sm text-amber-800">
                Your answers partially match expected responses. Focus on the
                questions with lower scores and review the subject matter more
                thoroughly.
              </p>
            </div>
          ) : (
            <div className="bg-red-50 p-4 rounded-lg border border-red-300">
              ...
              <p className="text-sm text-red-800">
                Your answers show significant gaps from expected responses.
                Consider reviewing the material and practicing answer
                formulation skills.
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900">
                {Math.round(
                  Math.max(
                    ...answerScore.results.map((r) => r.matching_percentage),
                  ),
                )}
                %
              </p>
      ...
      >
        ...
        <ul className="space-y-2 text-sm text-blue-800">
          <li>
            • <strong>Review Low Scores:</strong> Focus on questions where your
            match percentage is below 60%
          </li>
          <li>
            • <strong>Understand Expectations:</strong> Study what makes a good
            answer for similar questions
          </li>
          <li>
            • <strong>Practice Comprehensively:</strong> Ensure your answers
            cover all key points mentioned in expected responses
          </li>
          <li>
            • <strong>Check for Completeness:</strong> Add missing details or
            context to improve match rates
          </li>
          <li>
            • <strong>Learn from Matching Answers:</strong> Use your 100%
            matches as models for improvement
          </li>
        </ul>
      </motion.div>
    </motion.div>
