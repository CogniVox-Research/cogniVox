...
export function LiveTranscript({ entries, simulate = true }: LiveTranscriptProps) {
    ...
    return (
        <div className="rounded-xl border border-border bg-card shadow-sm flex flex-col" style={{ minHeight: '320px' }}>
            {/* Header */}
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
                <div>
                    <h2 className="text-base font-semibold text-card-foreground">Live Transcript</h2>
                ...
                {lines.length === 0 ? (
                    ...
                    lines.map((entry, idx) => {
                        ...
                        return (
                            <div
                                key={entry.id}
                                className={`flex items-start gap-3 transition-opacity duration-500 ${isLast ? 'opacity-100' : 'opacity-75'
                                    }`}
                            >
                                {/* Timestamp */}
                                ...
                                {/* Text */}
                                <p
                                    className={`text-sm leading-relaxed ${entry.isFinal
                                            ? isLast
